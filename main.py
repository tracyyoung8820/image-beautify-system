...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        # 虚化
        self.blurred.triggered.connect(self.__blurred)
        # 加边框
        self.addside.triggered.connect(self.__addside)
        # 倒影图
        self.reflection.triggered.connect(self.__addreflection)
        #超分辨率
        self.Superresolution.triggered.connect(self.__Superresolution)
        #马赛克
        self.actioncdzc.triggered.connect(self.__actioncdzc)
...
#模糊
    def __blurred(self):
        if self.__fileName:
            # 获取图片的数组
            img = self.__outImageRGB
            # 模糊图
            imgBlur = cv2.GaussianBlur(img, (151, 151), 5)
            # 展示
            self.__outImageRGB = imgBlur
            self.__drawImage(self.outImageView, self.__outImageRGB)
#加边框
    def __addside(self):
        # 打开图片文件
        if self.__fileName:
            ...
            # 显示带边框的图像
            self.__outImageRGB=img1
            self.__drawImage(self.outImageView, self.__outImageRGB)
#加倒影
    def __addreflection(self):
        if self.__fileName:
            # 获取图像的高度和宽度
            reflection_height = 0.5
            image=self.__outImageRGB
            height, width = image.shape[:2]

            # 计算倒影的高度
            reflection_height_px = int(reflection_height * height)

            # 创建带有倒影的画布
            reflected_image = cv2.flip(image, 0)
            reflected_image = cv2.resize(reflected_image, (width, reflection_height_px))

            # 将倒影叠加在原始图像的底部
            reflected_canvas = cv2.vconcat([image, reflected_image])
            self.__outImageRGB=reflected_canvas
            self.__drawImage(self.outImageView, self.__outImageRGB)
#超分辨率
    def __Superresolution(self):
        if self.__fileName:
           low_res_image = self.__outImageRGB

           # 创建基于深度学习的超分辨率模型
           sr = cv2.dnn_superres.DnnSuperResImpl_create()
           sr.readModel('ESPCN_x4.pb')  # 加载预训练的模型
           sr.setModel('espcn', 4)  # 设置模型，这里选择了 ESPCN 模型并指定放大倍数为 4

           # 使用超分辨率模型进行图像超分辨率处理
           high_res_image = sr.upsample(low_res_image)
           self.__outImageRGB=high_res_image
           # 显示结果
           self.__outImageRGB = high_res_image
           self.__drawImage(self.outImageView, self.__outImageRGB)
    #马赛克
    def __actioncdzc(self):
        if self.__fileName:
            image=self.__outImageRGB
            block_size = 10
            height, width = image.shape[:2]
